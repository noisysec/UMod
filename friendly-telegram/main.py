...
def main():  # noqa: C901
    ...
    while api_token is None:
        ...
        if web:
            ...
            print("- Havolani nusxalab brauzerdan kiring.")  # noqa: T001
            if not arguments.heroku_web_internal:
                port = str(web.port)
                if platform.system() == "Linux" and not os.path.exists(
                    "/etc/os-release"
                ):
                    print(f"- Havola --- http://127.0.0.1:{port}")
                else:
                    ipaddress = get("https://api.ipify.org").text
                    print(
                        f"- Havola --- http://{ipaddress}:{port} or http://localhost:{port}"
                    )
    ...
    for phone_id, phone in phones.items():
        ...
        try:
            ...
            print(
                "Raqam formati xato kiritilgan (+XX...)"  # noqa: T001
    ...
    if arguments.heroku:
        ...
        print(
            "Muvoffaqiyatli amalga oshirildi! .help buyrugʻini ishlatish mumkin.."
        )  # noqa: T001
        if web:
